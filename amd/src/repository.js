...
/**
 * Repository module to encapsulate all of the AJAX requests that can be sent for tdb.
 *
 * @module     paygw_tdb/repository
...
export const getState = (component, paymentArea, itemId, description) => {
    const request = {
        methodname: 'paygw_tdb_get_status',
        args: {
            component,
            paymentarea: paymentArea,
            itemid: itemId,
...
export const createRedirectUrl = async (component, paymentArea, itemId) => {
    const url = M.cfg.wwwroot + '/payment/gateway/tdb/redirect.php?' +
        'component=' + component +
        '&paymentarea=' + paymentArea +
        '&itemid=' + itemId;
    return url;
